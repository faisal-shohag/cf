...
var remaining;
$(function(){
 $.get('https://kontests.net/api/v1/codeforces',function(){})
 .done(function(contests){
  ...
  for(let i=0; i<contests.length; ++i){
   if(contests[i].name.includes("Codeforces Round") || contests[i].name.includes("Codeforces Educational") || contests[i].name.includes("Codeforces Global")){
   found = i;
    break;
   }
  }
   $('.contestName').html(`${contests[found].name}`);
   $('.register').html(`<a target="__blank" href="${contests[found].url}"><div class="rg">Register Now</div></a>`);
...
function dateTimeSet(time){
 ...
 // timerSet(fullDateTime);
 // timeContainer = fullDate + " " + fullTime;
 return {
   'date' : fullDate,
   'time' : fullTime
 }
